...
def dot_sp_mats(A, B, mkl=False, dense=False):
    if mkl:
        return dot_product_mkl(A, B, dense=dense)
    return A @ B

def eigsh_projector(p, log=True, log_interval=10000):
    '''Solve eigenvalue problem for matrix p. Return sparse matrix for 
    eigenvectors of matrix p.
    ...
    This function avoids solving numpy.eigh for duplicate block matrices.
    This function is efficient for matrix p composed of many duplicate 
    block matrices.
    '''
    ...
    for i,(s1,s2,ids) in enumerate(zip(s_begin,s_end,group.values())):
        ...
        if len(ids) > 1:
            ...
            if eigvecs is not None:
                ...
                col.extend([j for i in range(n_row) 
                              for j in range(col_id, col_id + n_col)])
...
def eigsh_projector_sumrule(p, log=True, log_interval=10000):
    '''Solve eigenvalue problem for matrix p. Return dense matrix for 
    eigenvectors of matrix p.
...
def solve_eig(p, rank_plus_one=True, log=True):
    '''deprecated: solve_eig is used only for test functions'''
