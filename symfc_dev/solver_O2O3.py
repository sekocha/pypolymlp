...
from pypolymlp.symfc_dev.solver import (
        solve_linear_equation, 
        solve_linear_ridge_equation
)
from pypolymlp.symfc_dev.solver_O2 import get_training_from_full_basis
from pypolymlp.symfc_dev.solver_O3 import csr_NNN333_to_NN33N3, get_batch_slice
from pypolymlp.symfc_dev.solver_O3 import set_2nd_disps
...
#    X2, y_all = get_training_from_full_basis(disps, forces, 
#                            full_basis_fc2.T.reshape((n_basis_fc2,N,N,3,3)))
#    t2 = time.time()
#    print(' training data (fc2):    ', t2-t1)
#
#    t1 = time.time()
#    compress_mat_fc3 = -0.5 * csr_NNN333_to_NN33N3(compress_mat_fc3, N)\
...
#        mat3y += X3.T @ y_batch
#        t06 = time.time()
#        print(t02-t01, t03-t02, t04-t03, t05-t04, t06-t05)
#        print(' solver_block:', end, ':, t =', t06-t01)
#
...
#    XTX[n_basis_fc2:,n_basis_fc2:] = (compress_eigvecs_fc3.T 
#                                      @ mat33 @ compress_eigvecs_fc3)
...
def get_training_exact(disps, 
    ...
    compress_mat_fc3 = csr_NNN333_to_NN33N3(compress_mat_fc3, N)\
                                        .reshape((NN33,-1)).tocsr()
    ...
    for begin, end in zip(begin_batch, end_batch):
        t01 = time.time()
        disps_batch = set_2nd_disps(disps[begin:end], 
                                    sparse=sparse_disps) @ c_perm_fc2
        t02 = time.time()
        ''' bottleneck part (use_mkl = False)'''
        X3 = dot_sp_mats(disps_batch, 
                         compress_mat_fc3, 
                         mkl=use_mkl, 
                         dense=True).reshape((-1,n_compr_fc3))
        ...
        if begin == 0:
            ...
            mat3y += X3.T @ y_batch
        t04 = time.time()
        #print(t02-t01, t03-t02, t04-t03)
        print(' solver_block:', end, ': t =', t04-t01)
...
def run_solver_sparse_O2O3(disps, 
                           forces, 
                           compress_mat_fc2, 
                           compress_mat_fc3, 
                           compress_eigvecs_fc2,
    ...
    coefs = solve_linear_ridge_equation(XTX, XTy, alpha=1e-8)
    n_basis_fc2 = compress_eigvecs_fc2.shape[1]
