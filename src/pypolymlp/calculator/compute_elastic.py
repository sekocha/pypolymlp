...
from pypolymlp.calculator.properties import (
    Properties, convert_stresses_in_gpa
)
from pypolymlp.calculator.str_opt.optimization_sym import MinimizeSym
from pypolymlp.utils.yaml_utils import print_array2d
        ...
        for i, j in ids:
            if self.__elastic_constants[i-1][j-1] > 1e-10:
                print('  c_' + str(i) + str(j) + ':', 
                    "{:.6f}".format(self.__elastic_constants[i-1][j-1]), file=f)
            else:
