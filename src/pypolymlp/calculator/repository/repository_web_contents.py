...
from pypolymlp.calculator.repository.utils.sphinx_utils import (
    text_to_csv_table, include_image
)
...
class PolymlpRepositoryWebContents:
    ...
    def __init__(self, path_data='./'):
        ...
        print('Repository web contents will be generated in', 
    ...
    def __get_num_structures(self):
        ...
        file1 = '/'.join([
            self.__polymlps[0]['id'], 'energy_dist', 'energy-train.dat']
        )
        cmd = "wc -l " + file1
        c1 = subprocess.check_output(cmd.split()).decode().split()[0]
        file2 = '/'.join([
            self.__polymlps[0]['id'], 'energy_dist', 'energy-test.dat']
        )
        cmd = "wc -l " + file2
    ...
    def run_summary(self):
        ...
        print('The current structure dataset comprises ' + str(n_st) + 
              ' structures. Procedures to generate structures'
              ' and estimate MLPs are found in'
              ' `A. Seko, J. Appl. Phys. 133, 011101 (2023)'
              ' <https://doi.org/10.1063/5.0129045>`_.', file=f)
        print('', file=f)

        include_image('summary/eqm_properties.png', 
                      title='Predictions using Pareto optimal MLPs', 
                      height=350, file=f)

        print('These properties are calculated for MLP equilibrium structures'
              ...
              ' sometimes differ from the one shown in the legend.', file=f)
        print('', file=f)
        print('The other properties predicted using each Pareto optimal MLP'
              ' are available from column **Predictions** '
              ' in the following table.', file=f)
        print('', file=f)

        min_rmse_f = min([float(d['rmse_force']) for d in self.__polymlps])
        if min_rmse_f > 0.05:
            print(self.__polymlps_id, 'shows large prediction errors.'
        ...
        print('Column "Time" shows the time required to compute the energy'
              ...
              file=f)
        print('Note that these MLPs should be carefully used for extreme'
    ...
    def run_polymlps(self):
        ...
        for d in self.__polymlps:
            if d['distribution']:
                path_data = '/'.join(
                    [self.__path_data, d['id'], 'polymlps']
                ) + '/'
                path_output = '/'.join(
                    [self.__path_output, 'polymlps', d['id']]
                ) + '/'
        ...
        tar = tarfile.open(
            path_output + 'polymlps-' + self.__polymlps_id + '.tar.gz', 'w:gz'
        )
        tar.add(path_output)
        tar.close()
    ...
    def __read_elastic_yaml(self, yamlfile):
        ...
        data_output = [
            [C['c_11'],C['c_12'],C['c_13'],C['c_14'],C['c_15'],C['c_16']],
            [C['c_12'],C['c_22'],C['c_23'],C['c_24'],C['c_25'],C['c_26']],
            [C['c_13'],C['c_23'],C['c_33'],C['c_34'],C['c_35'],C['c_36']],
            [C['c_14'],C['c_24'],C['c_34'],C['c_44'],C['c_45'],C['c_46']],
            [C['c_15'],C['c_25'],C['c_35'],C['c_45'],C['c_55'],C['c_56']],
            [C['c_16'],C['c_26'],C['c_36'],C['c_46'],C['c_56'],C['c_66']]
        ]
    ...
    def __read_icsd_yaml(self, yamlfile):
        ...
        data = [[data[i]['prototype'], 
                round(float(data[i]['mlp']), 5), 
                round(float(data[i]['dft']), 5)] for i in sort_ids]
    ...
    def run_predictions(self):
        ...
        for d in self.__polymlps:
            if d['distribution']:
                path_data = '/'.join(
                    [self.__path_data, d['id'], 'predictions']
                ) + '/'
                path_output = '/'.join(
                    [self.__path_output, 'predictions', d['id']]
                ) + '/'
                os.makedirs(path_output, exist_ok=True)
                ...
                path_dist = '/'.join(
                    [self.__path_data, d['id'], 'energy_dist']
                ) + '/'
                ...
                if os.path.exists(file_copied):
                    self.__copy(file_copied, path_output)
                    include_image(
                        'polymlp_icsd_pred.png', height=300, file=f,
                ...
                if os.path.exists(file_copied):
                    self.__copy(file_copied, path_output)
                    include_image(
                        'polymlp_phonon_dos.png', height=600, file=f,
                ...
                if os.path.exists(file_copied):
                    self.__copy(file_copied, path_output)
                    include_image(
                        'polymlp_thermal_expansion.png', height=400, file=f,
                ...
                if os.path.exists(file_copied):
                    self.__copy(file_copied, path_output)
                    include_image(
                        'polymlp_bulk_modulus.png', height=400, file=f,
