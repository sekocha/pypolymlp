...
class PolymlpRepositoryPrediction:
    ...
    def copy_files(self, path_output='./'):
        ...
        for pot_id, pot_info in self.__pot_dict.items():
            ...
            with open(path_output_single 
                      + "/energy_dist/energy-train.dat", "w") as outfile:
                ...
                for file1 in file_list:
                    ...
                    f.close()
                    for l in lines[1:]:
                        outfile.write(' '.join(l.split(' ')[:-2]) + '\n')

            file_list = glob.glob(pot_info['path'] + '/predictions/*test*')
            with open(path_output_single 
                      + "/energy_dist/energy-test.dat", "w") as outfile:
                outfile.write('# DFT(eV/atom), MLP(eV/atom)\n')
                for file1 in file_list:
                    f = open(file1, "r")
                    lines = f.readlines()
        ...
        for st, target in self.__target_list.items():
            ...
            write_poscar_file(target['structure'], 
                              filename=path_st + 'POSCAR', 
                              header=st)
        return self
    ...
    def run_icsd(self, path_output='./'):
        ...
        for pot_id, pot_info in self.__pot_dict.items():
            ...
            path_output_single = '/'.join(
                [path_output, pot_id, 'predictions']
            ) + '/'
            write_icsd_yaml(
    ...
    def run_properties(self, path_output='./', run_qha=False):
        ...
        for pot_id, pot_info in self.__pot_dict.items():
            for st, target in self.__target_list.items():
                ...
                path_output_single = '/'.join(
                    [path_output, pot_id, 'predictions', st]
                ) + '/'
