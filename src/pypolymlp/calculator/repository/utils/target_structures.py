...
def get_structure_list_alloy2(system, dir_DFT):
    ...
    del_list = []
    for st, (code, n_atom, _) in structure_list.items():
        poscar = dir_DFT + '/' + str(code) + '/POSCAR'
        if os.path.exists(poscar) == False:
            del_list.append(st)
    for st in del_list:
        del structure_list[st] 

    structure_dict = dict()
    for st, (code, n_atom, sup) in structure_list.items():
        structure_dict[st] = dict()
        structure_dict[st]['icsd_id'] = code
        structure_dict[st]['n_atom'] = n_atom
        structure_dict[st]['phonon_supercell'] = sup
...
def get_structure_list_element1(elements, dir_DFT):
    ...
    if len(set(elements) & set(['Te'])) > 0:
        structure_list = dict([('sc',  [43211, 1, [4,4,4]]),
        ...
        structure_list = dict([('fcc', [52914, 4, [4,4,4]]),
                               ('bcc', [76156, 2, [4,4,4]]),
                               ('hcp', [652876, 2, [4,4,4]]),
                               ('sc',  [43211, 1, [4,4,4]]),
                               ('diamond', [41979, 8, [4,4,4]]),
        ...
        structure_list = dict([('Sn(tI2)', [43216, 2, [4,4,4]]),
                               ('Sn', [236662, 4, [4,4,4]]),
                               ('Sm', [652633, 9, [6,6,1]]),
                               ('La', [52916, 4, [6,6,2]]),
                               ('sc',  [43211, 1, [4,4,4]]),
    ...
    for st, (code, n_atom, sup) in structure_list.items():
        ...
        if os.path.exists(vasprun):
            ...
            structure_dict[st]['n_atom'] = n_atom
            structure_dict[st]['phonon_supercell'] = sup
            structure_dict[st]['structure'] = st_dict
