...
def plot_icsd_prediction(icsd_dict_array, system, pot_id, 
                         path_output='./', dpi=300, 
...
def plot_energy(data_train, data_test, system, pot_id, 
                path_output='./', dpi=300):
    ...
    for i in range(2):
        ...
        ax[i][0].scatter(data_train[:,0],
                         data_train[:,1],
                         s=0.25, c='black',
                         alpha=1.0,
                         marker='.',
                         label='training')
        ax[i][1].scatter(data_test[:,0],
                         data_test[:,1],
                         s=0.25, c='orangered',
                         alpha=1.0,
                         marker='.',
...
def plot_eos(eos_dict, system, pot_id, 
             emin=None, path_output='./', dpi=300):
    ...
    fig.suptitle("Equation of state (" + system + ', '+ pot_id + ')', 
                 fontsize=10)
...
def plot_eos_separate(
    eos_dict, system, pot_id, 
    emin=None, path_output='./', 
    n_cols=3, figsize=(8,12), dpi=300, fontsize=10
):
    ...
    for i in range(n_cols):
        ax[-1][i].set_xlabel('Volume ($\mathrm{\AA}^3$/atom)',
                             fontsize=fontsize)
...
def plot_phonon(
    phonon_dict, system, pot_id, 
    path_output='./', dpi=300,
    n_cols=3, figsize=(8,8), fontsize=10,
):
...
def plot_phonon_qha_thermal_expansion(
    ...
    system, pot_id, 
    path_output='./', dpi=300,
    n_cols=3, figsize=(8,6), fontsize=10,
):
    ...
    fig, ax = plt.subplots(n_rows, n_cols, figsize = figsize)
    fig.suptitle("Thermal expansion (" 
    ...
    plt.savefig(path_output + '/polymlp_thermal_expansion.png',
                format='png', dpi=dpi)
    plt.savefig(path_output + '/polymlp_thermal_expansion.eps',
                format='eps')
    plt.clf()
    plt.close()
...
def plot_phonon_qha_bulk_modulus(
    ...
    plt.savefig(path_output + '/polymlp_bulk_modulus.png',
                format='png', dpi=dpi)
    plt.savefig(path_output + '/polymlp_bulk_modulus.eps',
                format='eps')
    plt.clf()
    plt.close()
