...
class HarmonicReal:
    ...
    def __get_distribution(self, t=1000, n_samples=100):
        ...
        disp_normal_coords = np.random.multivariate_normal(
                mean, cov, n_samples
        )
    ...
    def __calc_harmonic_potentials(self, t=1000, disps=None):
        ...
        if disps is None:
            ...
            pot_harmonic = [self.__harmonic_potential(d.T.reshape(-1)) 
                            for d in disps]
        ...
        if log:
            ...
            if len(outlier_ids) > 0:
                ...
                for i in sorted(outlier_ids):
                    print('- pot. energy (outlier ' + str(i),') =',
                          '{:f}'.format(energies[i]))
    ...
    def run(self, t=1000, n_samples=100, eliminate_outliers=True):
        ...
        energies, self.__forces = self.compute_polymlp_properties(
                self.__supercells
        )
        self.__energies_full = energies - self.__e0
        self.__eliminate_outliers()

        self.__energies_harm = self.__calc_harmonic_potentials(
                t=t, disps=self.__disps
        )
        return self
