...
if __name__ == '__main__':
    ...
    parser.add_argument('--p_step', 
                        type=float,
                        default=1.0,
                        help='pressure interval')
    parser.add_argument('--tc_min', 
                        type=float,
                        default=0.0,
                        help='minimum of transition temperature')
    parser.add_argument('--tc_max', 
                        type=float,
                        default=np.inf,
    ...
    for temp, gp_data in gp1.items():
        if temp in gp2:
            ...
            d2_train = [gp2_data[i] for i in range(len(gp2_data)) 
                        if i % 10 != 0]
            d2_test = [gp2_data[i] for i in range(len(gp2_data)) 
                        if i % 10 == 0]
            d1_train = np.array(d1_train)
            d1_test = np.array(d1_test)
            d2_train = np.array(d2_train)
            d2_test = np.array(d2_test)
            poly_diff, order_opt = find_optimal_order(d1_train, 
                                                      d1_test, 
                                                      d2_train, 
                                                      d2_test)
