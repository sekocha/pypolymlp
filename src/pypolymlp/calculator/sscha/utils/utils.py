...
class SummaryEOSYaml:
    ...
    def __init__(self, yaml_file):
        ...
        self.__fv = self.__data_dict(key='eos_data', 
                                     value_key='volume_helmholtz')
        self.__fv_fit = self.__data_dict(key='eos_fit_data', 
                                         value_key='volume_helmholtz')
        self.__gp_fit = self.__data_dict(key='eos_fit_data', 
                                         value_key='pressure_gibbs')
    ...
    def __equilibrium_data(self):
        ...
        self.__free_energy = [[d['temperature'], d['free_energy']]
                               for d in self.__data['equilibrium']]
        self.__bulk_modulus = [[d['temperature'], d['bulk_modulus']]
                                for d in self.__data['equilibrium']]
        self.__volume = [[d['temperature'], d['volume']]
                          for d in self.__data['equilibrium']]
