...
def print_eigvecs(
    lcomb, lproj, mproj, eigvecs, lm_indices, tol=1e-10, file=None
):
    ...
    for i, eig in enumerate(eigvecs.T):
        ...
        mcombs = [[m for l, m in lm] for lm in np.array(lm_indices)[nonzero]]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--minl', type=int, default=None, 
        help='Exclude l combinations composed of only l < minl.'
    )
    parser.add_argument(
        '--maxl', type=int, default=10, help='Maximum l value.'
    )
    parser.add_argument(
    ...
    for order in orders:
        ...
        for lcomb in lcomb_all:
            ...
            for mproj in range(-args.lproj, args.lproj + 1):
                ...
                print_eigvecs(
                    lcomb, args.lproj, mproj, eigvecs, lm_indices, file=f
                )
        f.close()
