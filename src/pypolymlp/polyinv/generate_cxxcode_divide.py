...
def print2d(array, varname=None, end=';', file=None):

    if varname is not None:
        print('    '+varname+' = {', file=file)
    else:
        print('        {', file=file)

    if len(array) > 1:
        for a1 in array[:-1]:
            print('        {'+', '.join(a1)+'},', file=file)

    print('        {'+', '.join(array[-1])+'}', file=file)
...
def print2dnewline(array, varname, file=None):
    ...
    print('        {'+',\n         '.join(array[-1])+'}', file=file)
    print('    };', file=file)
    print('', file=file)


def print3d(array, varname, file=None):

    print('    '+varname+' = {', file=file)

    if len(array) > 1:
        for a2 in array[:-1]:
            print2d(a2, varname=None, end=',', file=file)
    print2d(array[-1], varname=None, end='', file=file)
...
for i, (l_all,m_all,c_all) in enumerate(zip(
                                l_all_orders,m_all_orders,c_all_orders
                              )):

    order = str(i+1)
    f = open('gtinv_data_cpp' + order + '.cpp', 'w')
    print('void GtinvDataVer2Order' + str(i+1) + '::set_gtinv_info(){', file=f)
    print('', file=f)
    
    print2d(l_all, 'l_array_all', file=f)
    print2dnewline(c_all, 'coeffs_all', file=f)
    print3d(m_all, 'm_array_all', file=f)
