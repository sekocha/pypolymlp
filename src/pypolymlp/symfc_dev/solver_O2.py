...
def csr_NN33_to_N3N3(mat, N):
    ''' Reorder row indices in a sparse matrix (NN33->N3N3).
        Return reordered csr_matrix.
    '''
    NN33, nx = mat.shape
    mat = mat.tocoo()
    row = NN33_to_N3N3(mat.row, N)
    mat = csr_matrix((mat.data, (row, mat.col)), shape=(NN33,nx))
    return mat
...
def get_training(disps, 
    ...
    t1 = time.time()
    compress_mat = csr_NN33_to_N3N3(compress_mat, N).reshape((N3,-1)).tocsr()
    t2 = time.time()
    ...
    for begin, end in zip(begin_batch, end_batch):
        t01 = time.time()
        disps_batch = csr_matrix(disps[begin:end])
        X = dot_sp_mats(disps_batch, compress_mat, 
                        mkl=True, dense=True).reshape((-1,n_compr))
...
def solve(disps, forces, full_basis):
...
def run_solver_dense_O2(disps, forces, compress_mat, compress_eigvecs):
    ...
    coefs = solve(disps, forces, full_basis.T.reshape((n_basis,N,N,3,3)))
    return coefs

def run_solver_sparse_O2(disps, 
                         forces, 
                         compress_mat, 
                         compress_eigvecs,
                         batch_size=200):
