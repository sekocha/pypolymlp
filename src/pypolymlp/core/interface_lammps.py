...
class Dump:
    ...
    def __init__(self, name):
        ...
        data = np.array([[float(a) for a in line.split()] \
        ...
        self.axis[0][2], self.axis[1][2], self.axis[2][2] = xz, yz, zhi-zlo
...
#            [iatoms+1:iatoms+natom+1] for a in line.split()[1]]))
        n_types = max([int(a) for line in self.lines\
            [iatoms+1:iatoms+natom+1] for a in line.split()[1]])

        positions, self.n_atoms = \
            [[] for i in range(n_types)], [0 for i in range(n_types)]
        ...
        self.types =\
            [i for i, p in enumerate(positions) for j in range(len(p))]
