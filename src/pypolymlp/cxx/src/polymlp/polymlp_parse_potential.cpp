...
void parse_basic_params(std::ifstream& input, DataPolyMLP& pot){
    ...
    pot.fp.cutoff = get_value<double>(input);
    pot.fp.pair_type = get_value<std::string>(input);
    pot.fp.feature_type = get_value<std::string>(input);
...
void parse_params_conditional(std::ifstream& input, DataPolyMLP& pot){
    ...
    pot.fp.params_conditional = vector3i(pot.fp.n_type, vector2i(pot.fp.n_type));
    ...
    if (!input.eof() and n_type_pairs > 0){
        for (int i = 0; i < n_type_pairs; ++i){
            ...
            pot.fp.params_conditional[atomtypes[0]][atomtypes[1]] = param_indices;
        }
    }
    else {
        const int n_params = pot.fp.params.size();
        vector1i param_indices;
        for (int i = 0; i < n_params; ++i) param_indices.emplace_back(i);
        for (int i = 0; i < pot.fp.n_type; ++i){
            for (int j = 0; j <= i; ++j){
                pot.fp.params_conditional[j][i] = param_indices;
            }
        }
    }
}
...
bool parse_type_indices(std::ifstream& input, DataPolyMLP& pot, vector1i& type_indices){
    ...
    if (!input.eof()){
        type_indices = get_value_array<int>(input, pot.fp.n_type);
    }
    else {
        n_type_full = true;
        for (int i = 0; i < pot.fp.n_type; ++i) type_indices.emplace_back(i);
    }
    return n_type_full;
}
...
void parse_polymlp(
    ...
    parse_params_conditional(input, pot);

    // type_indices (optional)
    vector1i type_indices;
    bool n_type_full = parse_type_indices(input, pot, type_indices);
