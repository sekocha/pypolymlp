...
def get_lcomb_all(order=2, lproj=0, maxl=9, sym=False):
    f = open('lists/lcomb/lcomb-o'+str(order)+'-l'+str(lproj))
    l_all = []
    for line in f.readlines():
        lcomb = [int(l.replace(',','').replace('(','').replace(')','')) \
            for l in line.split()[1:]]
        if (sym == True):
            if (max(lcomb) <= maxl and len(set(lcomb))==1):
                l_all.append(lcomb)
        else:
            if (max(lcomb) <= maxl):
                l_all.append(lcomb)
...
def build_projector(lcomb, mcomb_all):
    ...
    obj = mlpcpp.Projector()
    obj.build_projector(lcomb, mcomb_all);

    return csc_matrix((obj.get_data(), (obj.get_row(), obj.get_col())), \
            shape=(size, size))
...
if __name__ == '__main__':
    ...
    if (args.sym==True and args.lproj == 0):
        fname = './lists/coeffs/coeffs-sym-o'\
            +str(args.order)+'-l'+str(args.lproj)
    else:
    ...
    lcomb_all = get_lcomb_all\
        (order=args.order, lproj=args.lproj, maxl=args.maxl, sym=args.sym)
