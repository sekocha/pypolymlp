...
def _func_adam(x, xty, coef, grad, sq_grad, alpha, beta1, beta2, learning_rate):
    """Function for Adam."""
    grad_add = x.T @ (x @ coef) + alpha * coef - xty
...
def solver_sgd(
    ...
    sq_grad = np.zeros(x.shape[1])
    norm = 1e10
    for i in range(max_iter):
        if i > 0 and norm < gtol:
            break
        coef, grad, sq_grad = _func_adam(
            x, xty, coef, grad, sq_grad, alpha, beta1, beta2, learning_rate
        )
