...
def compute_error(params_dict, 
    ...
    forces = np.array(list(itertools.chain.from_iterable(
                    [f.T.reshape(-1) for f in forces]
                     )))
    ...
    if log_force == False:
        ...
        rmse_f, true_f, pred_f = __compute_rmse(dft_dict['force'],
                                                forces,
                                                return_values=True)
    ...
    if log_stress == False:
        ...
        rmse_s, true_s, pred_s = __compute_rmse(dft_dict['stress'],
                                                stresses,
                                                normalize=normalize,
                                                return_values=True)
