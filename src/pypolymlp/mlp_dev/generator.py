...
from pypolymlp.mlp_dev.features import Features
from pypolymlp.mlp_dev.precondition import Precondition
...
#        self.__multi_dataset = False
    ...
    def compute_features(self):
        ...
        features_train = Features(
            self.__params_dict, self.__train_dict['structures']
        )
        self.__train_reg_dict = features_train.regression_dict

        features_test = Features(
            self.__params_dict, self.__test_dict['structures']
        )
        self.__test_reg_dict = features_test.regression_dict
        return self

    def apply_scales_weights(self):

        pre_train = Precondition(
            self.__train_reg_dict, self.__train_dict, 
            self.__params_dict, scales=None
        )
        pre_train.print_data_shape(header='Training data size')
        self.__train_reg_dict = pre_train.regression_dict

        pre_test = Precondition(
            self.__test_reg_dict, self.__test_dict, 
            self.__params_dict, scales=self.__train_reg_dict['scales']
        )
        pre_test.print_data_shape(header='Test data size')
        self.__test_reg_dict = pre_test.regression_dict
        return self
    ...
    @params_dict.setter
    def params_dict(self, params=None, infile=None):
