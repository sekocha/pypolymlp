...
from pypolymlp.core.dataset_utils import (
    DatasetDFT,
    permute_atoms,
...
def test_permute_atoms_with_spins1(structure_rocksalt):
    """Test for permute_atoms_with_spins."""
    ...
    order = np.array([0, 2, 1, 3, 4, 5, 6, 7])
    np.testing.assert_allclose(force_permute, force[:, order])
    np.testing.assert_allclose(st.positions, structure_rocksalt.positions[:, order])
    np.testing.assert_equal(st.n_atoms, [2, 2, 4])
    np.testing.assert_equal(st.types, [0, 0, 1, 1, 2, 2, 2, 2])
    np.testing.assert_equal(st.elements, np.array(structure_rocksalt.elements)[order])


def test_permute_atoms_with_spins2(structure_rocksalt):
    """Test for permute_atoms_with_spins."""
    structure_rocksalt_copy = copy.deepcopy(structure_rocksalt)
    structure_rocksalt_copy.types = np.array([0, 1, 0, 1, 2, 2, 2, 2])
    force = np.random.random(structure_rocksalt.positions.shape)
    ...
    order = np.array([4, 5, 6, 7, 0, 2, 1, 3])
    np.testing.assert_allclose(force_permute, force[:, order])
    np.testing.assert_allclose(st.positions, structure_rocksalt.positions[:, order])
